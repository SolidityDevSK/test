...
const HeroLeftFooter = () => {
   const {  allSaleDetailDomains,   totalSoldDomains, totalMintedDomains } = useContext(TransactionContext)
      ...
      <div className="opacity-80">
        <div className="flex flex-col justify-between">
          ...
          <div className="md:text-xl text-sm flex md:flex-row flex-col justify-between text-primary mt-4">
            <p>Sold Domains: <span className="text-white">{totalSoldDomains}</span></p>
            <p>Minted Domains: <span className="text-white">{totalMintedDomains.length}</span> </p>
            <p>Available for Sale: <span className="text-white">{allSaleDetailDomains.length}</span></p>
          </div>
        </div>
      </div>
  );
