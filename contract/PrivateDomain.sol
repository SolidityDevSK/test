...
contract PrivateDomain is ERC721 {
    ...
    function transferPriv() internal returns(bool status){
        return status = IERC20(privTokenAddress).transferFrom(msg.sender, address(this), privPrice*10**tokenDecimals);
    }

    function withdraw(address who) public onlyOwner returns(bool status){
        return status = IERC20(privTokenAddress).transfer(who, IERC20(privTokenAddress).balanceOf(address(this)));
    }
    ...
    function changeDomainBackgroundImage(string memory newBgHash, uint tokenId) public {
        string memory domainName = _getDomain[tokenId];
        require( msg.sender == _domainExists[domainName].domainOwner, "You do not have transaction authorization for this token.");
        _domainExists[domainName].ipfsHash = newBgHash;
    }
